 ...
 * Call Perplexity API
 */
const callPerplexity = async (prompt, maxTokens = 2048, useResearch = false) => {
  // Using 'sonar' model - works with current API key
  // Note: Docs say 'sonar' should be offline, but it performs web search successfully
  // May be aliased to sonar-online for Pro accounts
  const model = 'sonar';
  ...
  const requestBody = {
    ...
    temperature: 0.7,
    return_citations: true,  // Enable citations for research
    search_recency_filter: 'month',  // Limit to recent results (last 30 days)
  };
